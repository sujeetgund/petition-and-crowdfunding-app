...
export function CreatePetitionForm({ email }: { email: string }) {
  const [state, action, pending] = useActionState(createPetition, undefined);
  const router = useRouter();
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-2xl">Create a New Petition</CardTitle>
        <CardDescription>
          Fill out the form below to create a new petition. All fields are
          required.
        </CardDescription>
      </CardHeader>
      <CardContent>
